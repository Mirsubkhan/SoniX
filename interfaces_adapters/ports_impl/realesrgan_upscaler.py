...
class RealERSGANUpscaler(ImageUpscaler):
    def __init__(self):
        ...
        self.up_sampler = RealESRGANer(scale=4, model_path=model_path, model=self.model, tile=128, pre_pad=0, half=True, tile_pad=10)

    async def upscale_image(self, image: ndarray, fpath: Path) -> ndarray:
        height = image.shape[0]
        width = image.shape[1]
        outscale = 2 if height and width > 2000 else 4
